...
export default function WaitlistForm({ seekerId }: { seekerId: string }) {
  ...
  const handleJoin = async () => {
    if (!selectedTypes.length) { setStatus('Please select at least one class type.'); return }
  ...
  if (loading) return <p style={{ color: '#aaa', fontFamily: "'Afacad', sans-serif" }}>Loading...</p>
  ...
  return (
    <div>
      ...
      <div style={{ background: '#fff', borderRadius: 16, padding: 32, border: '1px solid #f0e8e0', maxWidth: 560, margin: '0 auto' }}>
          ...
          <>
            <PreferenceSection label="Class Types">
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, justifyContent: 'center' }}>
                {existing.class_types.map(t => <Pill key={t} label={t} active />)}
              </div>
            </PreferenceSection>
              ...
              <PreferenceSection label="Preferred Time">
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, justifyContent: 'center' }}>
                  {existing.time_preferences.map(t => (
                    <Pill key={t} label={TIME_PREFS.find(p => p.value === t)?.label ?? t} active />
                  ))}
                </div>
              </PreferenceSection>
            ) : null}
            ...
            <div style={{ display: 'flex', justifyContent: 'center', marginTop: 24 }}>
              <button onClick={() => { setEditing(true); setStatus('') }} style={outlineBtnStyle}>
                Edit Preferences
              </button>
            </div>
          </>
        ...
        {isEditMode && (
          <>
            <PreferenceSection label="Class Types" required>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, justifyContent: 'center' }}>
                {CLASS_TYPES.map(t => (
                  <Pill key={t} label={t} active={selectedTypes.includes(t)} onClick={() => toggleType(t)} />
                ))}
              </div>
            </PreferenceSection>

            <PreferenceSection label="Class Level" hint="optional — leave blank to match any level">
              <select
                value={classLevel}
            ...
            <PreferenceSection label="Preferred Time" hint="optional — select all that apply">
                ...
                {TIME_PREFS.map(t => (
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: 12,
                      border: '1.5px solid',
                      borderColor: timePref.includes(t.value) ? '#F35C20' : '#e8e0d8',
                      background: timePref.includes(t.value) ? '#fff3ee' : '#fafafa',
                  ...
                  >
                    <div style={{ fontSize: 14 }}>{timePref.includes(t.value) ? '✓ ' : ''}{t.label}</div>
                    <div style={{ fontSize: 11, color: timePref.includes(t.value) ? '#F35C20' : '#aaa', marginTop: 2 }}>{t.hint}</div>
                  </button>
                ))}
              </div>
            </PreferenceSection>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center', marginTop: 24 }}>
              <button onClick={existing ? handleUpdate : handleJoin} style={primaryBtnStyle}>
        ...
        {status && (
          <p style={{ marginTop: 16, fontSize: 14, textAlign: 'center', color: status.startsWith('✅') ? '#22c55e' : '#ef4444' }}>
            {status}
          </p>
        )}
      </div>
...
function PreferenceSection({ label, hint, required, children }: {
  label: string; hint?: string; required?: boolean; children: React.ReactNode
}) {
  return (
    <div style={{ marginBottom: 24 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, justifyContent: 'center', marginBottom: 12 }}>
        <p style={{ fontSize: 13, fontWeight: 700, color: '#444', textTransform: 'uppercase', letterSpacing: 0.8, margin: 0 }}>
          {label}
        </p>
...
const primaryBtnStyle: React.CSSProperties = {
  background: '#F35C20', color: '#fff', border: 'none',
  padding: '11px 28px', borderRadius: 100, fontSize: 15,
  fontFamily: "'Afacad', sans-serif", fontWeight: 600, cursor: 'pointer',
}

const outlineBtnStyle: React.CSSProperties = {
  background: 'transparent', color: '#F35C20', border: '1.5px solid #F35C20',
  padding: '11px 28px', borderRadius: 100, fontSize: 15,
  fontFamily: "'Afacad', sans-serif", fontWeight: 600, cursor: 'pointer',
}

const selectStyle: React.CSSProperties = {
  padding: '11px 14px', borderRadius: 10, border: '1.5px solid #e8e0d8',
  fontSize: 15, fontFamily: "'Afacad', sans-serif", outline: 'none',
  width: '100%', background: '#fafafa', boxSizing: 'border-box', color: '#111',
}