...
function Countdown({ createdAt, onExpire }: { createdAt: string; onExpire: () => void }) {
  ...
  return (
    <span style={{ fontSize: 13, fontWeight: 700, color: isUrgent ? '#ef4444' : '#F35C20' }}>
...
export default function NotificationInbox({ seekerId }: { seekerId: string }) {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('notif-inbox')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'notifications', filter: `seeker_id=eq.${seekerId}` }, fetchNotifs)
      .subscribe()
  ...
  return (
    <div>
      ...
      {/* Subtitle + Clear All — constrained to same width as cards */}
      <div style={{
        maxWidth: 720,
        margin: '0 auto 28px',
        display: 'flex',
        ...
        {notifs.length > 0 && (
          <button
            ...
            onMouseOut={e => { e.currentTarget.style.color = '#aaa'; e.currentTarget.style.borderColor = '#e8e0d8' }}
          >
            Clear All
          </button>
        )}
      ...
      {notifs.length === 0 && (
        <div style={{ textAlign: 'center', padding: '60px 24px', background: '#fff', borderRadius: 16, border: '1px solid #f0e8e0', maxWidth: 560, margin: '0 auto' }}>
          <div style={{ fontSize: 40, marginBottom: 12 }}>📭</div>
          <p style={{ color: '#aaa', fontSize: 15 }}>No notifications yet. Join the waitlist to get notified when a spot opens up.</p>
        </div>
      )}
        ...
        {notifs.map(n => (
          <div key={n.id} style={{
            ...
            <button
              ...
              style={{ position: 'absolute', top: 10, right: 12, background: 'none', border: 'none', fontSize: 16, color: '#ccc', cursor: 'pointer', lineHeight: 1, padding: 4 }}
            ...
            <p style={{ fontSize: 15, color: '#333', marginBottom: 12, lineHeight: 1.7, whiteSpace: 'pre-line' }}>{n.message}</p>

            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, flexWrap: 'wrap', marginBottom: n.status === 'pending' ? 14 : 0 }}>
              <span style={{
            ...
            {n.status === 'claimed' && claimedSpots[n.spot_id]?.claim_info && (
              <div style={{ textAlign: 'left', marginTop: 14, padding: '14px 16px', background: '#f0fdf4', borderRadius: 10, border: '1px solid #86efac' }}>
                <p style={{ fontWeight: 700, fontSize: 13, marginBottom: 6, color: '#166534' }}>Booking Info</p>
                <p style={{ fontSize: 13, whiteSpace: 'pre-wrap', color: '#333', margin: 0 }}>{claimedSpots[n.spot_id].claim_info}</p>
              </div>
            )}

            {n.status === 'pending' && (
              <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
                <button onClick={() => claim(n.id, n.spot_id, n.waitlist_entry_id)} style={claimBtnStyle}>
                  ✅ Claim Spot
                </button>
                <button onClick={() => rejectSpot(n.id, n.spot_id, n.waitlist_entry_id).then(fetchNotifs)} style={rejectBtnStyle}>
                  ✕ Not Interested
                </button>
              </div>
            )}
